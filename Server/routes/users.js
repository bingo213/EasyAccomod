  ...
  User.register(new User({ username: req.body.username, user_type: user_type }), req.body.password, (err, user) => {
    if (err) {
      ...
      user.save((err, user) => {
        ...
        Profile.create({user: user._id, email: req.body.email, fullname: req.body.fullname, active: activeProfile, update: updateProfile});
...
.get(authenticate.verifyUser, (req, res, next) => {
  ...
  .then((profile) => {
    if(profile){
      ...
      res.statusCode = 200;
      res.setHeader('Content-Type', 'application/json');
      res.json({success:false, message:'Not found Profile',profile: profile});
    }   
  })
})
  ...
  .then((user) => {
    if(user){
      if(user.user_type == 'rental' ) {
        res.statusCode = 403;
        res.setHeader('Content-Type', 'application/json');
        res.json({success:false, message:'You not have permission on Rental Account'});
      }
      else{
